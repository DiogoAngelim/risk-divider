...
export function updateRecommendedIcon(assets?: Asset[]): void {
  ensureD3()
    .then((d3) => {
      ...
      if (svg.empty()) {
        console.warn("updateRecommendedIcon: #recommended element not found");
        return;
      }
      ...
      console.log("updateRecommendedIcon debug:", {
        recommended,
        have,
      ...
      const iconSelection = svg.selectAll(".icon");
      if (iconSelection.empty()) {
        console.warn("updateRecommendedIcon: .icon element not found within #recommended");
        return;
      }
